...
Deterministic, source-grounded replies cover availability, role fit, contact, salary handling, private topics, and the VR embargo. Generative failures return an explicit temporary-unavailable response; they never fabricate a fallback.
...
- Owner-triggered delete/export controls. Automatic expiry is already enforced daily and during request/dashboard activity.
...
- Worker: `yahya-portfolio-retention`.
- Configuration: `wrangler.retention.jsonc`.
...
2. Open `/api/health`; expect `ready: true`, `ai: true`, `logging: true`, `privacyHashing: true`, and `scheduledRetention: true`.
...
1. Run `npm run check:release` and `git diff --check` before every release.
2. Confirm Cloudflare Pages deployed the intended commit and recheck `/api/health`.
3. Re-test authorized admin access after any Access policy, audience, hostname, or identity-provider change.
4. Verify the public AI/privacy notice, scheduled retention deployment, and D1 schema after infrastructure changes.
...
After cloning, run `npm ci` and `npm run check:release`, then verify the current production commit, `/api/health`, Access, and the retention Worker deployment. The highest-value product work after routine verification is the approval-based monthly knowledge dashboard. Update the StarLink record and rebuild the CV only when the official evidence arrives. VR remains locked until written clearance.
